...
def test_copy_shares_glyph_file() -> None:
    ...
    context = CmapContext({
        0x4E00: CmapGlyphVariants({None: glyph_file}),
    })
...
def test_merge_by_code_point_replaces_whole_variants_group() -> None:
    ...
    result = CmapContext({0x4E00: default_variants}).merge_by_code_point(
        CmapContext({0x4E00: flavored_variants}),
        conflict='replace',
    )
...
def test_merge_by_flavor_replaces_only_conflicting_flavor() -> None:
    ...
    target_variants = CmapGlyphVariants({None: default_file, 'zh_cn': old_file})
    source_variants = CmapGlyphVariants({'zh_cn': new_file})

    result = CmapContext({0x4E00: target_variants}).merge_by_flavor(
        CmapContext({0x4E00: source_variants}),
        conflict='replace',
    )
...
def test_get_glyph_sequence_sorts_by_flavor_then_code_point_and_deduplicates() -> None:
    ...
    first_variants = CmapGlyphVariants({None: shared_file, 'zh_cn': shared_file})
    second_variants = CmapGlyphVariants({None: default_file, 'zh_cn': shared_file})
...
def test_get_character_mapping_uses_context_code_points() -> None:
    ...
    assert context.get_character_mapping() == {0x42: 'u0041'}
